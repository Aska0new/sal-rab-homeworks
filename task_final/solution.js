...
function sendRequest(name, phone, address {street, house, entrance, floor, flat}, goods, sum) {
    ...
    data.client = `Клиент: ${name}, тел. ${phone}`;
    ...
    data.order.address = `Адрес доставки: ул. ${address.street}, дом ${address.house}, ${address.entrance} подъезд, ${address.floor} этаж, кв. ${address.flat}`;
    data.order.sum = sum;
